...
_random_cache = {"data": None, "timestamp": 0, "user_id": None}
...
class CalibreDB:
    ...
    def check_valid_db(cls, config_calibre_dir, app_db_path, config_calibre_uuid):
        ...
        except (OperationalError, sqliteOperationalError, AttributeError):
            return False, False
    ...
    def setup_db(cls, config_calibre_dir, app_db_path):
        ...
        try:
            ...
            with engine.begin() as connection:
                connection.execute(text('PRAGMA cache_size = -20000;'))
            ...
            conn = engine.connect()
        except (OperationalError, sqliteOperationalError, OSError) as ex:
    ...
    def fill_indexpage_with_archived_books(self, page, database, pagesize, db_filter, order, allow_show_archived,
                                           join_archive_read, config_read_column, *join):
        pagesize = pagesize or current_user.per_page
        if current_user.show_detail_random():
            now = time.time()
            uid = int(current_user.id)
            if (_random_cache["data"] is not None
                    and _random_cache["user_id"] == uid
                    and (now - _random_cache["timestamp"]) < RANDOM_CACHE_TTL):
                randm = _random_cache["data"]
            else:
                ...
                _random_cache["data"] = randm
                _random_cache["timestamp"] = now
                _random_cache["user_id"] = uid
        else:
            randm = false()
