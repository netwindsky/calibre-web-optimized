...
class TaskGenerateCoverThumbnails(CalibreTask):
    def __init__(self, book_id=-1, task_message=''):
        ...
        self._calibre_session = None

    BATCH_SIZE = 500
    MAX_WORKERS = 16

    def _get_calibre_session(self):
        if self._calibre_session is not None:
            return self._calibre_session
        calibre_dir = getattr(config, 'config_calibre_dir', None)
        if not calibre_dir:
            self.log.error('Calibre library path not configured')
            return None
        dbpath = os.path.join(calibre_dir, "metadata.db")
        if not os.path.isfile(dbpath):
            self.log.error('Calibre database not found: %s', dbpath)
            return None
        from sqlalchemy import create_engine as ce
        from sqlalchemy.orm import sessionmaker
        engine = ce('sqlite:///{0}'.format(dbpath), echo=False,
                    connect_args={'check_same_thread': False})
        Session = sessionmaker(bind=engine)
        self._calibre_session = Session()
        return self._calibre_session
    ...
    def run(self, worker_thread):
        if use_IM and self.stat != STAT_CANCELLED and self.stat != STAT_ENDED:
            ...
            if self.book_id != -1:
                ...
                total_generated = self._run_batch()
            if total_generated == 0:
                self.self_cleanup = True
        self._handleSuccess()
        self.app_db_session.remove()
        if self._calibre_session:
            self._calibre_session.close()

    @staticmethod
    def _resize_cover(book_path, resolution, output_path):
        book_cover_filepath = os.path.join(config.get_book_path(), book_path, 'cover.jpg')
        ...
        with Image(filename=book_cover_filepath) as img:
            ...
            if img.height > height:
                ...
                copyfile(book_cover_filepath, output_path)
        return True

    def _run_batch(self):
        calibre_session = self._get_calibre_session()
        if not calibre_session:
            self.log.error('Failed to connect to calibre database')
            return 0

        total_count = calibre_session.query(db.Books).filter(db.Books.has_cover == 1).count()
        self.log.info('Starting thumbnail generation: %d books, %d workers', total_count, self.MAX_WORKERS)
        if total_count == 0:
            return 0

        processed = 0
        total_generated = 0
        self.message = N_('Scanning %(count)s books (%(workers)s threads)',
                          count=total_count, workers=self.MAX_WORKERS)
        offset = 0
        while self.stat != STAT_CANCELLED and self.stat != STAT_ENDED:
            books = calibre_session.query(db.Books).filter(db.Books.has_cover == 1) \
                .order_by(db.Books.id).offset(offset).limit(self.BATCH_SIZE).all()
            if not books:
                self.log.info('No more books at offset %d, stopping', offset)
                break
            ...
            calibre_session.expunge_all()

            work_items = []
            for bd in book_data:
                book_thumbnails = self.get_book_cover_thumbnails(bd['id'])
                resolutions = list(map(lambda t: t.resolution, book_thumbnails))
                ...
                for resolution in missing_resolutions:
                    ...
                    try:
                        ...
                        continue
                    output_path = self.cache.get_cache_file_path(thumbnail.filename, constants.CACHE_TYPE_THUMBNAILS)
                    work_items.append((bd['path'], resolution, output_path))
                for thumbnail in book_thumbnails:
                    need_update = False
                    if bd['last_modified'] and bd['last_modified'].replace(tzinfo=None) > thumbnail.generated_at:
                        need_update = True
                    ...
                    if need_update:
                        ...
                        try:
                            ...
                            continue
                        output_path = self.cache.get_cache_file_path(thumbnail.filename, constants.CACHE_TYPE_THUMBNAILS)
                        work_items.append((bd['path'], thumbnail.resolution, output_path))

            with ThreadPoolExecutor(max_workers=self.MAX_WORKERS) as executor:
                futures = {}
                for book_path, resolution, output_path in work_items:
                    f = executor.submit(self._resize_cover, book_path, resolution, output_path)
                    futures[f] = book_path
                for future in as_completed(futures):
                    ...
                    processed += 1
                    self.progress = processed / total_count if total_count else 1.0
                    try:
                        if future.result():
                            total_generated += 1
                    except Exception as ex:
                        self.log.debug('Error generating thumbnail: %s', ex)
                    self.message = N_('Generated %(count)s thumbnails (%(done)s/%(total)s, %(workers)s threads)',
                                      count=total_generated, done=processed, total=total_count,
                                      workers=self.MAX_WORKERS)

            offset += self.BATCH_SIZE
            self.app_db_session.rollback()
            self.log.info('Batch done: offset=%d, processed=%d, generated=%d', offset, processed, total_generated)

        self.log.info('Thumbnail generation complete: %d generated out of %d processed', total_generated, processed)
        return total_generated
