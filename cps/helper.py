...
def _add_cover_cache_headers(response, book_id, last_modified_ts=None):
    response.headers['Cache-Control'] = 'public, max-age=86400, stale-while-revalidate=604800'
    if last_modified_ts:
        etag = f'cover-{book_id}-{int(last_modified_ts)}'
...
def _generate_thumbnail_async(book, resolution):
    import threading
    def _gen():
        try:
            _generate_thumbnail_on_demand(book, resolution)
        except Exception:
            pass
...
def _generate_thumbnail_on_demand(book, resolution):
    if not use_IM:
        return None
    try:
        ...
        if not thumbnail:
            return None
        cache = fs.FileSystem()
        cache_dir = cache.get_cache_file_dir(thumbnail.filename, CACHE_TYPE_THUMBNAILS)
        os.makedirs(cache_dir, exist_ok=True)
        with WandImage(filename=cover_path) as img:
            target_h = int(255 * resolution)
            w_ratio = target_h / float(img.height)
            new_w = int(img.width * w_ratio)
            img.resize(new_w, target_h, filter='lanczos')
            save_path = os.path.join(cache_dir, thumbnail.filename)
            img.save(filename=save_path)
        return save_path
    except Exception as ex:
        log.debug('On-demand thumbnail generation failed for book %s: %s', book.id, ex)
        return None


def get_book_cover_internal(book, resolution=None):
    if book and book.has_cover:
        last_modified_ts = book.last_modified.timestamp() if book.last_modified else 0
        ...
        if resolution:
            ...
            if thumbnail:
                ...
                if cache.get_cache_file_exists(thumbnail.filename, CACHE_TYPE_THUMBNAILS):
                    resp = send_from_directory(cache.get_cache_file_dir(thumbnail.filename, CACHE_TYPE_THUMBNAILS),
                                               thumbnail.filename)
                    return _add_cover_cache_headers(resp, book.id, last_modified_ts)
                else:
                    _generate_thumbnail_async(book, resolution)
