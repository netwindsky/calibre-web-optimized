...
def get_per_page():
    return current_user.per_page if current_user.is_authenticated else config.config_books_per_page
...
def render_books_list(data, sort_param, book_id, page):
    ...
    if data == "rated":
        ...
        offset = int(int(pp) * (page - 1))
        return render_search_results(term, offset, order, pp)
    elif data == "advsearch":
        term = json.loads(flask_session.get('query', '{}'))
        offset = int(int(pp) * (page - 1))
        return render_adv_search_results(term, offset, order, pp)
    else:
...
def render_rated_books(page, book_id, order):
    if current_user.check_visibility(constants.SIDEBAR_BEST_RATED):
        pp = get_per_page()
        entries, random, pagination = calibre_db.fill_indexpage(page, pp,
                                                                db.Books,
...
def render_hot_books(page, order):
    if current_user.check_visibility(constants.SIDEBAR_HOT):
        ...
        if current_user.show_detail_random():
            ...
            random = false()
        pp = get_per_page()
        off = int(int(pp) * (page - 1))
        all_books = ub.session.query(ub.Downloads, func.count(ub.Downloads.book_id)) \
            .order_by(*order[0]).group_by(ub.Downloads.book_id)
        hot_books = all_books.offset(off).limit(pp)
        entries = list()
        ...
        pagination = Pagination(page, pp, num_books)
        return render_title_template('index.html', random=random, entries=entries, pagination=pagination,
...
def books_list(data, sort_param, book_id, page):
    ...
    if pp and pp in ["30", "60", "90"]:
        current_user.set_view_property("per_page", "value", int(pp))
...
def list_books():
    ...
    limit = int(request.args.get("limit") or get_per_page())
...
def get_cover(book_id, resolution=None):
    try:
        ...
        log.error("Cover error for book %s resolution %s: %s", book_id, resolution, e)
        import traceback
